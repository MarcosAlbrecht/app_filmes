...
class GetMovieDetailsCommand extends _$GetMovieDetailsCommand {
  ...
  Future<void> execute(int movieId) async {
    final resultado = await ref.read(tmdbRepositoryProvider).getMovieDetail(movieId);
    state = switch (resultado) {
      Success<MovieDetail>(:final value) => AsyncData(value),

      Failure() => AsyncError(Exception('Erro ao buscar detalhes de filme'), StackTrace.current),
    };
  }
}
